...
export interface UploadFileV2Result {
    /** File type, e.g. "image", "video", "audio" */
    type: string;
    /** Public download URL (valid for ~1 day) */
    downloadUrl: string;
    /** Internal fileName for use in workflow nodeInfoList fieldValue */
    fileName: string;
...
export async function createTask(
    ...
    const response = await axios.post(
        `${RUNNINGHUB_BASE_URL}/task/openapi/create`,
        requestBody,
        {
            headers: {
 ...
 */
export async function queryTaskOutputs(taskId: string): Promise<TaskOutputResult> {
...
export interface PhotoRestoreOptions {
    /** Optional extra text prompt for the restoration */
    text?: string;
    /** CN strength: 0-1, lower = cleaner scratch removal, higher = less deformation. Default: 0.4 */
    cnStrength?: number;
    /** Output size in megapixels. Default: 1.6 (approx 1024x1600) */
    outputSize?: number;
}
 ...
 */
export async function submitPhotoRestore(
    imageUrl: string,
...
export interface TextToSpeechOptions {
    /** The text content to read. Required. */
    text: string;
    /** Tone description, e.g. "萝莉少女声音". Default: "萝莉少女声音" */
    voiceDescription?: string;
    /** Language. e.g. "自动", "中文", "英文", etc. Default: "自动" */
    language?: string;
}
...
export async function createTaskV2(
    ...
    const response = await axios.post(
        `${RUNNINGHUB_BASE_URL}/openapi/v2/run/ai-app/${workflowId}`,
        requestBody,
        {
            headers: {
                'Content-Type': 'application/json',
...
export interface ImageToVideoOptions {
    /** Positive prompt describing the desired motion/animation. Default uses a preset Chinese prompt. */
    positivePrompt?: string;
    /** Negative prompt describing what to avoid. Default uses a preset Chinese negative prompt. */
    negativePrompt?: string;
    /** Max resolution (pixels). Default: 912 */
    maxResolution?: number;
    /** Duration of the generated video in seconds. Default: 5 */
    duration?: number;
    /** Frame rate of the generated video. Default: 16 */
    frameRate?: number;
    /** Random seed. If not provided or <= 0, a random seed will be generated. */
    seed?: number;
}
...
export interface NovelToScriptOptions {
    /** The novel chapter text to convert. Required. */
    novelText: string;
    /** LLM temperature. Default: 0.6 */
    temperature?: number;
    /** Random seed for LLM. If not provided, a random seed will be generated. */
    seed?: number;
}
 ...
 * Returns the taskId for tracking.
 */
export async function submitNovelToScript(
    options: NovelToScriptOptions,
): Promise<string> {
