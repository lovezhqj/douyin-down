 ...
 */
async function seedDefaultQuotaConfigs() {
    for (const cfg of DEFAULT_QUOTA_CONFIGS) {
        await query(`INSERT INTO quota_config (biz_code, biz_name, daily_free_limit, daily_max_limit)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (biz_code) DO NOTHING`, [cfg.bizCode, cfg.bizName, cfg.dailyFreeLimit, cfg.dailyMaxLimit]);
    }
 ...
 */
export async function getTodayUsageCount(openid, bizCode) {
    const result = await query(`SELECT COUNT(*) AS cnt FROM ai_tasks
         WHERE openid = $1 AND biz_code = $2
           AND created_at >= ((NOW() AT TIME ZONE 'Asia/Shanghai')::date || ' 00:00:00 Asia/Shanghai')::timestamptz`, [openid, bizCode]);
    return parseInt(result.rows[0].cnt, 10);
}
/**
 * Get quota configuration for a specific biz_code.
 */
export async function getQuotaConfig(bizCode) {
    const result = await query(`SELECT * FROM quota_config WHERE biz_code = $1`, [bizCode]);
    return result.rows.length > 0 ? result.rows[0] : null;
}
/**
 * Get all quota configurations.
 */
export async function getAllQuotaConfigs() {
    const result = await query(`SELECT * FROM quota_config ORDER BY id ASC`);
    return result.rows;
}
/**
 * Create or update a quota configuration.
 */
export async function upsertQuotaConfig(bizCode, bizName, dailyFreeLimit, dailyMaxLimit) {
    const result = await query(`INSERT INTO quota_config (biz_code, biz_name, daily_free_limit, daily_max_limit)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (biz_code)
         DO UPDATE SET biz_name = $2, daily_free_limit = $3, daily_max_limit = $4, updated_at = NOW()
         RETURNING *`, [bizCode, bizName, dailyFreeLimit, dailyMaxLimit]);
    return result.rows[0];
}
/**
